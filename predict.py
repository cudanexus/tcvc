...
import os
# os.environ["AWS_ACCESS_KEY_ID"] = "ADD AWS S3 ID here"
# os.environ["AWS_SECRET_ACCESS_KEY"] = "ADD AWS S3 key here"
...
class Predictor(BasePredictor):
    def upload_to_s3(self,file, bucket_name, object_name):
    ...
    def upload_org_video(self,filename,bucket_name):
        self.upload_to_s3(filename, bucket_name, "upload_videos/"+filename)
        return "org Uploded Video"

    def upload_process_video(self,filename, bucket_name):
        s3_key = "processed_videos/" + filename
        self.upload_to_s3(filename, bucket_name, s3_key)
    ...
    def calculate_psnr_folders(self,input_path, GT_path, interval_length, logger):
    ...
    def save_imglist(self,k, end_k, output_dir, img_list, logger, img_paths):
    ...
    def main(self,video):
        ...
        Input_dataset_folder = "./input_videos"
        ...
        img_list = sorted(glob.glob(os.path.join("./images/", "*.png")))
    ...
    #             out_ab_rs = F.interpolate(out_ab, size=[H, W], mode="bilinear")
            ...
            self.save_imglist(k, k+len(out_rgb_img), save_subfolder, out_rgb_img, logger, img_paths)
    ...
    def predict(self, url: str = Input(description='Insert Video URL' )) -> ModelOutput:
        bucket_name="videos-tcvc"

        filename = self.download_video(url)
        if filename is not None:
            input_video = "./input_video/"+filename
            output_images = "./images/out_%d.png"

            cmd = ["ffmpeg", "-i", input_video, "-vf", "fps=0.5", output_images]
            subprocess.call(cmd)
            self.upload_org_video(input_video,bucket_name)
            image_pattern=self.main(filename)
            # exit()

            # Convert images to video
            image_pattern = image_pattern+"/out_%d.png"
            output_video = video_name = str(uuid.uuid4())+"_video.mp4"
            print("---------------------------------Image pattern",image_pattern)

            cmd = ["ffmpeg", "-framerate", "20", "-i", image_pattern,"-c:v", "libx264", "-pix_fmt", "yuv420p", output_video]
            subprocess.call(cmd)
            link=self.upload_process_video(output_video,bucket_name)



            return ModelOutput(video_path=Path(f"{str(output_video)}"),s3_url=link)
        # else:
